...
export function calculateShiftHours(
  ...
  // A range that ends before it starts is treated as crossing midnight
  // rather than a negative duration — this only checks if the clock wraps,
  // not the `overnight` flag itself.
...
// "H:MM" display, e.g. so users enter "1:30" instead of 90 raw minutes.
...
// Returns undefined for incomplete input (e.g. "1:") so callers can leave
// the field's own text alone until it parses.
...
// Clears fields whose owning toggle is off rather than persisting
// stale/hidden data.
...
// Earliest enabled start and latest enabled end across every day/range —
// a summary, not a promise every one of them matches it exactly.
