...
// Test fixtures only — the app no longer ships any seeded schedules (the
// store starts empty, see `schedules-store.ts`). These records stay because
// the rotation suite reads them as its worked examples; nothing in `src/`
// outside a `*.test.*` file may import them.
//
