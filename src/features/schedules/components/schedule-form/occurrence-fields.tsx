...
import { RecurrenceExceptionFields } from './recurrence-exception-fields'
...
// Step 4: occurrence — a universal step for every regular type (fixed,
// flexible, rotate), split into its three components: frequency, end, and
// exception settings.
export function OccurrenceFields({ disabled }: OccurrenceFieldsProps) {
  ...
  return (
    <div className='space-y-4'>
      ...
      <Card className='gap-3 py-4'>
        <CardHeader className='px-4'>
          <CardTitle className='text-sm font-medium'>End</CardTitle>
        </CardHeader>
        <CardContent className='px-4'>
      ...
      <Card className='gap-3 py-4'>
        ...
        <CardContent className='px-4'>
          <RecurrenceExceptionFields disabled={disabled} />
        </CardContent>
      </Card>
    </div>
  )
